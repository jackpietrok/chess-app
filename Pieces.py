...
class Pawn(Piece):
	...
	def get_moves(self,board,pos):
		...
		col = pos[1];
		if(this.team == "white"):
			if(row != 0 and (board[row-1][col+1] == None or board[row-1][col+1].team != "white")):
				arr += (row-1,col+1);
			if(row != 7 and (board[row+1][col+1] == None or board[row+1][col+1].team != "white")):
				arr += (row+1,col+1);
		else:
			if(row != 0 and (board[row-1][col-1] == None or board[row-1][col-1].team != "black")):
				arr += (row-1,col-1);
			if(row != 7 and (board[row+1][col-1] == None or board[row+1][col-1].team != "black")):
				arr += (row+1,col-1);
		return arr;
	...
	def get_moves(self,board,pos):
		...
		while(temp[0] != 0 and temp[1] != 0):
			if(board[temp[0]][temp[1]] != None):
				if(board[temp[0]][temp[1]] != this.team):
					arr += temp;
				break;
			else:
				arr += temp;
	...
	def get_moves(self,board,pos):
		...
		for i in range(row-2,row+3,4):
			for j in range(col-1,col+2,2):
				if(i >= 0 and i <= 7 and j <= 7 and j >= 0 and (board[i][j] == None or board[i][j].team != this.team)):
					arr += (i,j);
		...
		for i in range(row-1,row+2):
			for j in range(col-1,col+2):
				if((i == row and j == col) or i < 0 or i > 7 or j < 0 or j > 7):
					continue;
				elif(board[i][j] == None or board[i][j].team != this.team):
					arr += (i,j);
		return arr;
